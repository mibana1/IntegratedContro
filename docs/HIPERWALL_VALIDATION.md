...
## 2026-09-14 저장 배치·시나리오 확장 검증

- LIVE 읽기 기반 배치 저장·갱신·삭제, 저장 배치 추가 표시와 DeviceCommand → WaitUntil → ShowLayout 혼합 실행을 구현했다.
- 저장은 쓰기 명령을 보내지 않고, 접수 시 배치 버전·Controller·항목·인스턴스 ID를 고정한다.
  배치 갱신 후 원 snapshot 유지, 교대 후 실행 지속, Hiperwall 예약과 수동 편집 차단을 확인했다.
- 조건 단계가 쓰기 없이 반복 조회하고 다른 장비 작업을 허용하는지, 최신 관측/ACK/오래된 관측을 구분하는지,
  제한시간·Continue·취소 경합·재시작 자동 재개 차단을 검사했다.
- 콘텐츠/Zone 누락·Shadow 역할의 사전 검증 실패는 Continue로 우회하지 않는다.
  일부 전송 불확실은 남은 표시와 후속 단계를 차단하며, 개별 인스턴스 결과를 원 작업 이력에 보존한다.
  재시작 시 미전송·전송 중·부분 ACK를 구분하며 일부 ACK만으로 배치 전체 성공을 표시하지 않는다.
- 신규 서비스 회귀 19개 포함 전체 **197개 통과**, 전체 빌드 **경고 0/오류 0**.
  근거: artifacts/test-results/scenario-regression.trx.
- 기존 로그인·운영·조명·Hiperwall 편집·포인터·Zone·교대·이력/백업 WPF 전체 회귀와 새 저장 배치/시나리오 검증을 통과했다.
  새 검증은 저장 → 혼합 단계 편집/순서 → 실행, 수동 표시, 대기 중 교대·취소, 로그아웃 초기화와 1180×860 렌더링을 포함한다.
  근거: artifacts/ui-smoke/scenario-extensions-result.txt, saved-layouts.png, scenario-extensions.png.
- 격리 HTTPS 호스트·가짜 Controller에서 수행했다. 아래 사용자가 확인한 기존 기본 연동과 이번 확장의 검증 범위를 구분한다.
  운영 배포본 교체·운영 호스트 재시작·실제 Controller 시험 명령은 수행하지 않았다.
- 현재 저장 배치는 계속 추가 표시만 지원한다. 시간 지정 닫기·15초 임시 테스트와 레이어/투명도/라벨 복원은 후속 범위다.

## 2026-09-14 실제 기본 연동 확인 완료 (사용자 확인)

- 사용자가 **기본 조회·추가·변경·닫기**의 실제 Hiperwall 동작 확인을 완료했다고 알렸다.
- 현재 기본 실제 연동 확인 단계는 완료다. 이후 기능 확장은 이 확인을 기준으로 진행한다.
- 이 기록의 근거는 사용자 확인이다. 제품/펌웨어 버전, 세부 변경 명령별 결과와 시험 로그는 제공되지 않았다.
- 아래 이전 기록의 실제 Controller 미검증 표시는 각 구현·자동화 검사 당시의 범위다.
  두 PC·Enterprise·장애/응답 유실·재시작 복구 등 별도 항목은 이번 확인 범위에 포함하지 않는다.
...
## 초기 구현 당시 실제 Controller 요청 시도 기록
...
가짜 서버 성공을 실제 Hiperwall 연결 성공으로 보고하지 않는다. 초기 조회 구현 당시에는 영상벽 표시 상태를 바꾸는 요청을 구현·실행하지 않았다.
현재 기본 조회·추가·변경·닫기의 실제 동작 확인 상태는 문서 상단의 사용자 확인 기록을 따른다.
