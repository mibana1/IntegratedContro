...
- 연결 설정은 설치된 App/server-startup.json에 저장한다. 기본 재설치는 이를 덮어쓰지 않는다.
  연결 경로를 다시 지정하면 이전 파일은 .previous로 남긴다.
- 앱의 접속 주소·인증서 지문 설정은 기존 사용자 프로필을 사용한다. 새 PC에서 저장 후 앱을 다시 열면 서버 시작 확인을 수행한다.
- 기존 DB, 계정, 인증서 개인 키, DPAPI 파일, 현장 MediaMTX 비밀번호는 설치물에 포함하지 않는다.
  새 서버의 계정 초기 설정은 동봉 안내의 ControlHost setup 절차를 이용한다.
