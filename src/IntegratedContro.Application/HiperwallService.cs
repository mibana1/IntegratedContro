...
public sealed partial class ControlService
{
    ...
    private Session HiperwallReaderSession(string token)
    {
        ...
        Require(!_state.FencedSessions.Contains(session.Info.Id), "session_fenced", "이전 세션이 차단되었습니다. 다시 로그인하세요.", 403);
    ...
    public HiperwallSettingsView GetHiperwallSettings(string token)
    {
        lock (_gate)
        {
            HiperwallReaderSession(token); Admin(_state, token);
            return _state.Hiperwall is { } c ? Settings(c) : new(0, "", "", HiperwallAuthentication.Token, "", 3000, false);
        }
    }
    public HiperwallView GetHiperwallStatus(string token)
    {
        lock (_gate) { HiperwallReaderSession(token); return JsonDefaults.Copy(_hiperwallView ?? EmptyHiperwall()); }
    }
    public HiperwallSettingsView SaveHiperwallSettings(string token, SaveHiperwallRequest request)
    {
        lock (_gate)
        {
            ...
            var session = Owner(_state, token, request.Generation); Admin(_state, token);
            ...
            Audit(next, session.Info.UserId, "HiperwallSettingsSaved", $"version={next.Hiperwall.Version}");
            Persist(next);
            _hiperwallSequence++;
    ...
    private void CancelHiperwallSession(Guid sessionId)
    {
    ...
    public async Task<HiperwallView> RefreshHiperwallAsync(string token, bool connectionTest, CancellationToken ct)
    {
        ...
        lock (_gate)
        {
            ...
            if (connectionTest) Admin(_state, token);
            if (_state.Hiperwall is null)
            {
                if (connectionTest)
                {
                    var next = JsonDefaults.Copy(_state);
                    Audit(next, session.Info.UserId, "HiperwallConnectionTest", "version=0; result=NotConfigured");
                    Persist(next);
                }
        ...
        try
        {
            ...
            lock (_gate)
            {
                ...
                Require(_state.Hiperwall?.Version == config.Version, "hiperwall_settings_changed", "설정이 변경되어 이전 연결의 응답을 폐기했습니다.");
                query.Token.ThrowIfCancellationRequested();
                var success = reading.State == HiperwallConnectionState.Connected ? Now : previousSuccess;
                ...
                {
                    var next = JsonDefaults.Copy(_state);
                    Audit(next, session.Info.UserId, "HiperwallConnectionTest", $"version={config.Version}; result={reading.State}");
                    Persist(next);
                }
        ...
        catch (DomainException e) when (e.Code == "credential_store_unavailable")
        {
            ...
            {
                HiperwallReaderSession(token);
                Require(_state.Hiperwall?.Version == config.Version, "hiperwall_settings_changed", "설정이 변경되어 이전 연결의 결과를 폐기했습니다.");
                var result = FailedHiperwall(HiperwallConnectionState.ConnectionFailed, e.Message, previousView);
                if (sequence == _hiperwallSequence) _hiperwallView = result;
                if (connectionTest)
                {
                    var next = JsonDefaults.Copy(_state);
                    Audit(next, session.Info.UserId, "HiperwallConnectionTest", $"version={config.Version}; result=ConnectionFailed");
                    Persist(next);
                }
                return result;
            }
        ...
        catch (OperationCanceledException)
        {
            lock (_gate)
            {
                HiperwallReaderSession(token);
                Require(_state.Hiperwall?.Version == config.Version, "hiperwall_settings_changed", "설정이 변경되어 이전 연결의 응답을 폐기했습니다.");
                var state = ct.IsCancellationRequested || _stopping ? HiperwallConnectionState.ConnectionFailed : HiperwallConnectionState.TimedOut;
                ...
                {
                    var next = JsonDefaults.Copy(_state);
                    Audit(next, session.Info.UserId, "HiperwallConnectionTest", $"version={config.Version}; result={state}");
                    Persist(next);
                }
                return result;
            }
