...
internal sealed class HostAuthority
{
    ...
    internal Lease ReadLease()
    {
        var lease = JsonDefaults.Copy(_state.Lease);
    ...
    internal void Audit(HostState s, Guid? user, string action, string detail) =>
        s.Audit.Add(AuditPresentation.Enrich(new(Now, user, action, detail), s));
    internal Session Authenticate(string token)
    {
    ...
    internal Account User(HostState s, Session session) => s.Accounts.Single(a => a.Id == session.Info.UserId);
    internal Session Owner(HostState s, string token, long generation)
    {
    ...
    internal Session Admin(HostState s, string token)
    {
        var session = Authenticate(token);
    ...
    internal void CheckConnectionUnsafe()
    {
