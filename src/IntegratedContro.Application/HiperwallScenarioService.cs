...
{
    public StepSnapshot ResolveDisplay(StateContext context, AccountView user, ScenarioStep step)
    {
    ...
    public string? RevalidateDisplay(StateContext context, Job job, StepSnapshot step)
    {
        var s = _host.For(context);
        var invalid = RevalidateJob(s.SiteId, s.Scenarios, job, _host.Now);
    ...
    private static void RequireHiperwallScenarioAvailable(HiperwallStateScope state)
    {
    ...
    private bool ScenarioAllowsDisplay(HiperwallDisplayJob display)
    {
        ...
        var parent = _host.Current.Jobs.SingleOrDefault(j => j.Id == id);
        return parent is { Active: true, CancelRequestedAt: null } && display.ScenarioStepIndex is { } index &&
            ...
            RevalidateDisplay(_host.Current, parent, parent.Snapshot.Steps[index]) is null &&
            !display.Targets.Any(t => t.OpenState is HiperwallSendState.Rejected or HiperwallSendState.Unknown);
    }
    public void ValidateScenarioAdmission(StateContext context)
    {
    ...
    public async Task<StepExecutionResult?> PollScenarioDisplayAsync(Guid id, int index, StepSnapshot step, CancellationToken ct)
    {
        ...
        using (_host.Open())
        {
            var parent = FindJob(_host.Current.Jobs, id);
            var existing = _host.Current.HiperwallDisplays.SingleOrDefault(d => d.ScenarioJobId == id && d.ScenarioStepIndex == index);
        ...
        try
        {
            ...
            using (_host.Open())
            {
                ...
                var next = _host.Draft(); var parent = FindJob(next.Jobs, id);
                ...
                var user = _host.FindAccount(next, parent.Snapshot.RequestedBy)!;
                ...
                _jobs.ReportDisplayWaiting(next, parent.Id, index, "저장 배치 표시 접수 / Controller 응답 대기", admitted: true);
    ...
    private StepExecutionResult? ReadScenarioDisplayResult(Job parent, int index, HiperwallDisplayJob display)
    {
        ...
        _jobs.ReportDisplayWaiting(next, parent.Id, index, display.Summary);
        _host.Commit(next); return null;
    }
    internal void RecoverEdits(StateContext context)
    {
