...
internal sealed partial class CameraService
{
    ...
    public bool SyncCamera(string token, CameraActionRequest request) => _host.Change(s =>
    {
        var session = _host.Owner(s, token, request.Generation); _host.Admin(s, token); MediaAvailable();
        ...
        s.Cameras[s.Cameras.IndexOf(camera)] = camera with { Provisioning = CameraProvisioning.Pending,
            Message = "재동기화 접수 · 경로 준비 대기", NextSyncAt = _host.Now };
    ...
    public bool DeleteCamera(string token, CameraActionRequest request) => _host.Change(s =>
    {
        var session = _host.Owner(s, token, request.Generation); _host.Admin(s, token); MediaAvailable();
        var camera = FindCamera(s, request);
        if (request.Force)
        {
            ...
            s.CameraCleanup.Add(new(Guid.NewGuid(), s.Media!, camera.StreamPath, camera.SourceCredentialId,
                session.Info.UserId, session.Info.UserName, _host.Now));
            s.Cameras.Remove(camera);
            _host.Audit(s, session.Info.UserId, "CameraForceDeleted", $"camera={camera.Id}; path={camera.StreamPath}");
        }
        else
        {
            ...
            s.Cameras[s.Cameras.IndexOf(camera)] = camera with { Version = camera.Version + 1, DeleteRequested = true,
                Provisioning = CameraProvisioning.Deleting, NextSyncAt = _host.Now, Message = "경로 삭제 결과 대기 · 등록 유지" };
            _host.Audit(s, session.Info.UserId, "CameraDeleteRequested", $"camera={camera.Id}");
        }
        return true;
    });
    public bool RetryCameraCleanup(string token, long generation) => _host.Change(s =>
    {
        var session = _host.Owner(s, token, generation); _host.Admin(s, token);
        s.CameraCleanup = s.CameraCleanup.Select(c => c with { NextAttemptAt = _host.Now }).ToList();
        _host.Audit(s, session.Info.UserId, "CameraCleanupRequested", $"count={s.CameraCleanup.Count}");
        return true;
    });
                ...
                if (cleanup is not null || camera is not null)
                {
                    ...
                    if (cleanup is not null)
                    {
                        ...
                        if (failure is null)
                        {
                            ...
                            _host.Audit(next, item.RequestedBy, "CameraPathCleaned", $"path={item.StreamPath}");
                        }
                        else next.CameraCleanup[next.CameraCleanup.IndexOf(item)] = item with { Attempts = item.Attempts + 1,
                            NextAttemptAt = _host.Now.AddSeconds(30), Message = failure };
                    }
                        ...
                        current.Version == camera.Version && next.Media?.Version == config.Version)
                    {
                        if (failure is null && camera.DeleteRequested)
                        {
                            ...
                            _host.Audit(next, null, "CameraDeleted", $"camera={camera.Id}; path={camera.StreamPath}");
                        }
                        else
                        {
                            ...
                            next.Cameras[next.Cameras.IndexOf(current)] = current with { Provisioning = status, Message = message,
                                UpdatedAt = _host.Now, NextSyncAt = status == CameraProvisioning.DeleteFailed ? DateTimeOffset.MaxValue : _host.Now.AddSeconds(30),
