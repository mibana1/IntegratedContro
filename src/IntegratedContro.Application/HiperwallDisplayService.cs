...
public sealed partial class ControlService
{
    ...
    public HiperwallDisplayView GetHiperwallDisplays(string token)
    {
        lock (_gate)
        {
            ...
            var recent = _state.HiperwallDisplays.TakeLast(100).Select(j => j.Request.RequestId).ToHashSet();
            return JsonDefaults.Copy(new HiperwallDisplayView(_state.HiperwallLayouts.ToArray(),
                _state.HiperwallDisplays.Where(j => j.Outstanding || recent.Contains(j.Request.RequestId)).Reverse().ToArray()));
        }
    }
    ...
    public SavedHiperwallLayout SaveHiperwallLayout(string token, SaveHiperwallLayoutRequest request) => Change(s =>
    {
        ...
        var layout = new SavedHiperwallLayout(request.Id, request.ExpectedVersion + 1, request.ConfigurationVersion,
            request.Name.Trim(), JsonDefaults.Copy(request.Placements), request.Duration, Now);
        s.HiperwallLayouts.RemoveAll(l => l.Id == layout.Id); s.HiperwallLayouts.Add(layout);
        Audit(s, session.Info.UserId, "HiperwallLayoutSaved", $"layout={layout.Id}; version={layout.Version}");
        return layout;
    });
    public bool DeleteHiperwallLayout(string token, DeleteHiperwallLayoutRequest request) => Change(s =>
    {
        ...
        Audit(s, session.Info.UserId, "HiperwallLayoutDeleted", $"layout={request.Id}; 접수된 표시 유지");
        return true;
    });
    public async Task<HiperwallDisplayJob> DisplayHiperwallAsync(string token, HiperwallDisplayRequest request, CancellationToken ct)
    {
        ...
        lock (_gate)
        {
            ...
            if (_state.HiperwallDisplays.SingleOrDefault(j => j.Request.RequestId == request.RequestId) is { } prior)
            {
            ...
            Require(!_state.HiperwallEdits.Any(e => e.Request.RequestId == request.RequestId), "request_conflict", "LIVE 요청 ID가 이미 사용되었습니다.");
        }
        ...
        try
        {
            ...
            lock (_gate)
            {
                DisplayOwner(token, request.Generation, request.ConfigurationVersion); RequireHiperwallScenarioAvailable(_state); RequireNoSlotRestore(); config = _state.Hiperwall!;
                ...
                if (request.LayoutId is { } id)
                {
                    ...
                    var layout = _state.HiperwallLayouts.SingleOrDefault(l => l.Id == id);
            ...
            lock (_gate)
            {
                ...
                var session = DisplayOwner(token, request.Generation, request.ConfigurationVersion); RequireHiperwallScenarioAvailable(_state); RequireNoSlotRestore();
                ...
                var job = new HiperwallDisplayJob { Request = request, Requester = session.Info, Endpoint = config.Endpoint,
                    Name = name, Duration = duration, AcceptedAt = Now,
                    CloseAt = duration.EffectiveSeconds is { } seconds ? Now.AddSeconds(seconds) : null,
                    Targets = commands.Select(c => new HiperwallDisplayTarget { Command = c, NextAttemptAt = Now }).ToList() };
                next.HiperwallDisplays.Add(job);
                Audit(next, session.Info.UserId, "HiperwallDisplayAccepted", $"request={request.RequestId}; targets={commands.Length}");
                Persist(next); return JsonDefaults.Copy(job);
            }
        }
        finally { _hiperwallAdmission.Release(); }
    }
    public HiperwallDisplayJob StopHiperwallDisplay(string token, JobActionRequest request) => Change(s =>
    {
        var session = Owner(s, token, request.Generation);
        Require(HiperwallPermission(User(s, session)), "hiperwall_scope", "전체 장비 제어 권한이 필요합니다.", 403);
