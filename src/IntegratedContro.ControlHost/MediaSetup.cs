...
internal static class MediaSetup
{
    ...
    public static async Task InitializeAsync(string[] args, string dataPath)
    {
        ...
        if (state.LocalMediaChange is not null) throw new InvalidOperationException("관리자 화면에서 영상 서버 변경의 적용 확인 또는 복구를 먼저 완료하세요.");
        ...
        ManagedMediaSetup? saved = null;
        if (File.Exists(manifestPath))
        {
            using var file = File.OpenRead(manifestPath);
            if (file.Length > 4096) throw new InvalidDataException("영상 서버 자동 설정 기록을 확인하세요.");
            saved = JsonSerializer.Deserialize<ManagedMediaSetup>(file, JsonDefaults.Options)
                ?? throw new InvalidDataException("영상 서버 자동 설정 기록을 확인하세요.");
            if (saved.Version != 1 || saved.ConfigurationVersion < 1 || saved.CredentialId == Guid.Empty ||
        ...
        MediaConfiguration Configuration(ManagedMediaSetup setup) => new(setup.ConfigurationVersion, $"http://127.0.0.1:{api}", $"http://127.0.0.1:{hls}",
        ...
        if (passwords is null || passwords.ApiPassword is not { Length: >= 12 and <= 512 } || passwords.HlsPassword is not { Length: >= 12 and <= 512 } ||
            passwords.ApiPassword == passwords.HlsPassword)
            throw new InvalidDataException("영상 서버 보호 계정 정보를 확인하세요.");
        var yaml = ManagedMediaConfiguration.ConfigurationText(saved, passwords);
        if (File.Exists(configPath))
        {
            if (new FileInfo(configPath).Length > 16384 || File.ReadAllText(configPath).Replace("\r\n", "\n") != yaml.Replace("\r\n", "\n"))
