...
public sealed partial class MainViewModel
{
    ...
    private bool RoleInUse(RoleBinding role) => _state?.Jobs.Any(j => j.Active &&
        j.Snapshot.Steps.Any(s => s.Role?.Id == role.Id)) == true;
    private string RoleUnassignmentHint(RoleBinding role)
    {
        if (_state?.RoleUnassignmentSupported != true) return "배정 해제에는 최신 ControlHost가 필요합니다.";
        if (RoleInUse(role)) return "진행 중 작업에서 사용 중입니다. 작업·교대에서 완료를 확인하거나 취소한 뒤 해제하세요.";
        var definitions = _state.Scenarios.Where(s => s.Steps.Any(step => step.RoleId == role.Id)).ToArray();
    ...
    private void RefreshAssignedRoleRows()
    {
        ...
        {
            AssignedRoles.Remove(row); _commands.Remove(row.UnassignCommand); row.UnassignCommand.Raise();
        }
        foreach (var role in roles)
            if (!AssignedRoles.Any(row => row.Binding == role))
                AssignedRoles.Add(new(role, Command(() => UnassignRole(role), () =>
                    CanConfigure && _state?.RoleUnassignmentSupported == true &&
                    SelectedDevice?.Id == role.DeviceId && Roles.Contains(role) && !RoleInUse(role)),
                    () => RoleUnassignmentHint(role)));
        foreach (var row in AssignedRoles) row.Refresh();
    }
    private async Task UnassignRole(RoleBinding role)
    {
        await Client.Post<bool>("/api/roles/unassign", new UnassignRoleRequest(Generation, role.Id, role.DeviceId, role.Version));
        await Refresh();
        if (SelectedDevice?.Id == role.DeviceId && RoleName.Trim() == role.Id) LoadAssignedRoleName();
        Message = $"역할 배정 해제 완료: {role.Id}. 장비와 저장된 시나리오 정의는 유지됩니다.";
    }
}
